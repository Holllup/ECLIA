 ...
 */
export function ensureSystemInstructionFiles(rootDir = findProjectRoot(process.cwd())) {
    const { templatePath, localPath, createdTemplate, createdLocal } = ensureTemplateFiles(rootDir, SYSTEM_INSTRUCTION_FILE, SYSTEM_INSTRUCTION_LOCAL_FILE, DEFAULT_SYSTEM_INSTRUCTION_FILE_CONTENT);
    return { rootDir, systemPath: templatePath, localPath, createdSystem: createdTemplate, createdLocal };
}
export function readSystemInstruction(rootDir = findProjectRoot(process.cwd())) {
...
export function writeSystemInstructionLocal(rootDir, text) {
...
export function renderSystemInstructionTemplate(template, vars) {
